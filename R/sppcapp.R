spcapp <- function (Y, nPcs, MaxIter = 10000 ,TolFun = 1e-6,TolX = 1e-6)
{
    k        <- nPcs
    Y        <- scale(Y, scale = F)
    Y        <- t(Y)
...
#    v        <- 0.048375054285105
    ...
    ppcaOutput  <- ppcaSensible(Y, W, v, traceS, MaxIter, TolFun, TolX)
    ...
    res[["method"]]   <- "ppcaNet"
    return(res)
    #return(list(W = results$W, v = results$v, nloglk = results$nloglk))
